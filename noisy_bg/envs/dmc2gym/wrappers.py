...
from noisy_bg.envs.dmc2gym import natural_imgsource
...
class DMCWrapper(core.Env):
    def __init__(
        ...
        if from_pixels:
            self._observation_space = spaces.Box(
                low=0, high=255,
                shape=[frame_stack, height, width], dtype=np.uint8
            )
        else:
            ...
            if img_source == "color":
                ...
                if img_source == "images":
                    self._bg_source = natural_imgsource.RandomImageSource(
                        shape2d, files,
                        grayscale=False, total_frames=total_frames
                    )
    ...
    def step(self, action):
        ...
        return self._get_obs(), reward, done, extra
