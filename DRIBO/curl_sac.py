...
from DRIBO.encoder import make_encoder
...
class CURL(nn.Module):
    ...
    def __init__(
        self, obs_shape, z_dim, batch_size,
        critic, critic_target, output_type="continuous"
    ):
...
class CurlSacAgent(object):
    ...
    def __init__(
        ...
        if self.encoder_type == 'pixel':
            ...
            self.CURL = CURL(
                obs_shape, encoder_feature_dim, self.curl_latent_dim,
    ...
    def update(self, replay_buffer, L, step):
        if self.encoder_type == 'pixel':
            obs, action, reward, \
                next_obs, not_done, cpc_kwargs = replay_buffer.sample_cpc()
        else:
            obs, action, reward, \
        ...
        if step % self.cpc_update_freq == 0 and self.encoder_type == 'pixel':
            obs_anchor, obs_pos = cpc_kwargs["obs_anchor"], \
                cpc_kwargs["obs_pos"]
